  ...
  return (
    <>
      ...
      <div>
        ...
        {/* Banner : Gallery */}
        <div id='gallery' className='bg-[#f0008c] border-b-4 border-[#08b14a] h-[150px] lg:h-[200px] xl:h-[250px] flex'>
        ...
        {/* Gallery Images */}
        {!loading && images.length > 0 && <div className={`columns-2  md:columns-3 lg:columns-4 xl:columns-5 my-5 px-3 md:px-10`}>
          {images.map((image) => (
        ...
        {images.length === 0 && !loading && <div className=' h-screen flex items-center justify-center'>
          <p className='text-white animate-bounce font-poppins text-xl md:text-2xl text-center font bold'>No Images Found :( </p>
        </div>}
        {error && !loading && <div className=' h-screen flex items-center justify-center'>
